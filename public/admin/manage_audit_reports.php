...
<head>
    <meta charset="UTF-8">
    <title>Manage Audit Reports | Admin</title>
    ...
    <main class="admin-main">
        <?php if(isset($_GET['success'])): ?>
            <div style="background: #d4edda; color: #155724; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
                <i class="fas fa-check-circle"></i> Audit Report published successfully!
            </div>
        <?php endif; ?>

        <div class="admin-card">
            <h2><i class="fas fa-file-pdf" style="color: #e74c3c;"></i> Post An Audit Report</h2>
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit" name="submit_upload" class="btn-submit">Publish Audit Report</button>
            </form>
        </div>

        <h3>Existing Audit Reports</h3>
        <table class="report-list-table">
            ...
            <tbody>
                <?php
                $res = mysqli_query($conn, "SELECT * FROM audit_reports ORDER BY report_year DESC");
